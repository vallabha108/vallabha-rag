...
def run_prompts(project: str, location: str, corpus_display_name: str, top_k: int = 5) -> Path:
    ...
    for p in prompts:
        ...
        print(f"  [{p['id']}] generating answer with {EVAL_MODEL} ...", flush=True)
        gen = client.models.generate_content(
            model=EVAL_MODEL,
            contents=ANSWER_PROMPT.format(context=context_block, question=p["question"]),
        )
        results.append(
            {
                "id": p["id"],
...
def evaluate(project: str) -> Path:
    ...
    judge = GeminiModel(model=EVAL_MODEL, project=project, location=GENAI_LOCATION, use_vertexai=True)
    opik_model = f"vertex_ai/{EVAL_MODEL}"
...
def recommend(project: str) -> Path:
    ...
    for case in data["cases"]:
        ...
        gen = client.models.generate_content(
            model=EVAL_MODEL,
            contents=MITIGATION_PROMPT.format(
    ...
    if diagnoses:
        ...
        gen = client.models.generate_content(
            model=EVAL_MODEL,
            contents=OVERALL_PROMPT.format(
                top_k=run.get("top_k"), model=run.get("generation_model"), diagnoses="\n\n".join(diagnoses)
            ),
        )
        data["recommendations"] = (gen.text or "").strip()
    else:
